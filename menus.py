...
def menu(con, header, options, width, screen_width, screen_height):
    ...
    # calculated height for header after auto-wrap and one line per option
...
def inventory_menu(con, header, player, inventory_width, screen_width, screen_height):
    ...
    if len(player.inventory.items) == 0:
        ...
        for item in player.inventory.items:
            if player.equiment.main_hand == item:
                options.append('{0} (on main hand)'.format(item.name))
            elif player.equiment.off_hand == item:
...
def main_menu(con, background_image, screen_width, screen_height):
    ...
    libtcod.console_print_ex(0, int(screen_width/2), int(screen_height - 2), libtcod.BKGND_NONE, libtcod.CENTER, 'By Griffin Poole')
...
def character_screen(player, character_screen_width, character_screen_height, screen_width, screen_height):
    ...
    libtcod.console_print_rect_ex(window, 0, 6, character_screen_width, character_screen_height, libtcod.BKGND_NONE, libtcod.LEFT, 'Maximum HP: {0}+{1}'.format(player.fighter.base_max_hp, player.fighter.max_hp - player.fighter.base_max_hp))
    libtcod.console_print_rect_ex(window, 0, 7, character_screen_width, character_screen_height, libtcod.BKGND_NONE, libtcod.LEFT, 'Attack: {0}+{1}'.format(player.fighter.base_power, player.fighter.power - player.fighter.base_power))
    libtcod.console_print_rect_ex(window, 0, 8, character_screen_width, character_screen_height, libtcod.BKGND_NONE, libtcod.LEFT, 'Defense: {0}+{1}'.format(player.fighter.base_defense, player.fighter.defense - player.fighter.base_defense))
