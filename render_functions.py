...
def get_data_under_mouse(mouse, entities, fov_map):
    ...
    for entity in entities:
        if entity.x == x and entity.y == y and libtcod.map_is_in_fov(fov_map, entity.x, entity.y):
            if entity.fighter:
                data.append('{0} HP:{1}/{2} DEF:{3} POW:{4}'.format(entity.name, entity.fighter.hp, entity.fighter.max_hp, entity.fighter.defense, entity.fighter.power))
            else:
...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height, bar_width, panel_height, panel_y, mouse, colors, game_state):
    ...
    if fov_recompute:
        for y in range(game_map.height):
            for x in range(game_map.width):
                # Tracks whether tile blocks sight
    ...
    # Iterate through entities, draw all
    ...
    libtcod.console_print_ex(panel, 1, 0, libtcod.BKGND_NONE, libtcod.LEFT, get_data_under_mouse(mouse, entities, fov_map))
